...
//   const API_KEY = import.meta.env.VITE_NEWS_API_KEY;
  ...
  return (
    <div className="live-article-container mx-auto max-w-screen-xl relative">
      {loading ? (
        <p className="text-center text-lg font-medium">Loading latest news...</p>
      ) : error ? (
        <p className="text-center text-lg text-red-500">{error}</p>
      ) : latestNews ? (
        <>
          <img
            src={latestNews.urlToImage || "https://via.placeholder.com/1200x580"}
          ...
          <span className="absolute top-[16px] left-[16px] bg-white text-xs font-medium px-[10px] py-[5px] rounded-sm flex items-center">
            <VscCircleFilled style={{ color: "red", marginRight: "5px" }} /> Live Updates
          </span>
          <div className="absolute bottom-[20px] left-[20px] bg-white p-4 rounded-sm w-[90%] shadow-lg">
